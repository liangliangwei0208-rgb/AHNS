...
- `AHNS Server Sleep` 和 `AHNS Server Wake And Start` 当前均已停用，交流/直流自动睡眠设为“从不”；不要在没有用户明确要求时重新启用待机策略。睡眠、唤醒脚本和任务仍保留备用。
...
- `main.py`：主计算入口，手动运行时生成市场 RSI 图和海外/全球基金估算；总入口中用 `--skip-rsi` 跳过 RSI，只写正式基金估算缓存。
...
- `fund_region_allocation.py`：晨星公开基金页的股票地区分布图入口。`--all` 全量分页，`--fund-code 012922 --refresh` 手动直连单基金，`--auto` 只重绘地区权重或披露日期变化的页面。请求固定 `trust_env=False`，不继承 VPN HTTP/SOCKS 环境代理；TUN 全局接管时仍要在 VPN 软件中为 `www.morningstar.cn` 配直连。自动图输出在 `output/fund_region_allocation/latest/`，手动图输出在 `manual/`，会纳入收盘流程邮件。
- `safe_fund.py`：只读基金估算缓存，生成安全版海外/全球收盘观察图；自动总入口只在北京时间 06:00-13:40 运行它。早间盘后/富途夜盘窗口与收盘窗口重叠时，会先运行 `main.py --skip-rsi`、`fund_holding_change.py --auto`、`fund_region_allocation.py --auto` 和 `safe_fund.py`，再运行对应实时观察；晚间盘前/盘中窗口不会输出收盘观察图，但 RSI 图仍会全天生成。
...
- 晨星股票地区分布图：
  - `output/fund_region_allocation/latest/1_海外基金地区分布.png`（自动检测到变化时按固定页码更新）
  - `output/fund_region_allocation/manual/012922_基金地区分布.png`（手动指定基金时生成）
- 盘前观察图：
...
```powershell
& D:\anaconda\envs\py310\python.exe -m py_compile .\service_main.py .\service_runner.py .\service_gui.py .\service_command_watcher.py .\sync_repos.py .\github_gitee_sync.py
```
...
- 持仓变化图在短视频顶部被遮挡：优先增大 `tools/configs/fund_holding_change_style_configs.py` 的 `top_margin_px`；想提升像素清晰度改 `canvas_width_px`，无需直接修改绘图函数。
- 晨星地区图直连失败：先确认日志含“晨星直连”；普通代理环境会被 `trust_env=False` 忽略。如 VPN 使用 TUN 接管全局网络，需要在 VPN 软件内为 `www.morningstar.cn` 添加直连规则，再用 `fund_region_allocation.py --fund-code 012922 --refresh` 验证。
