...
# 1.20 表示把“行情有效的已披露持仓股占净值比例”放大 20%，
# 同时从纳斯达克100补偿仓位中扣除对应权重，保持总估算权重不超过 100%。
OVERSEAS_VALID_HOLDING_BOOST = 1.20
...
def _apply_stale_market_zero_policy(
    ...
    stale_market_zero_markets=("CN", "HK"),
):
    """
    节假日海外估值防重复口径。

    对 A股 / 港股等指定市场，如果本次拿到的行情交易日早于估算日，
...
def fetch_cn_security_return_pct_daily_with_date(code, lookback_days=90):
    """
    获取 A股 / A股ETF / 场内基金最新完整交易日日线涨跌幅，并返回实际交易日。
...
def fetch_cn_security_return_pct_daily(code, lookback_days=90):
    """获取 A股 / A股ETF / 场内基金最新完整交易日日线涨跌幅，兼容旧接口。"""
    r_pct, trade_date, source = fetch_cn_security_return_pct_daily_with_date(code=code, lookback_days=lookback_days)
    return r_pct, source



def fetch_hk_return_pct_akshare_daily_with_date(code, lookback_days=90):
    """
    使用 AKShare 港股历史日线获取最新交易日涨跌幅，并返回实际交易日。
    ...
    try:
        df = ak.stock_hk_hist(symbol=hk_code, period="daily", start_date=start_date, end_date=end_date, adjust="")
    except TypeError:
    ...
    if pct_col is not None:
        ...
        if len(valid_idx) > 0:
            last_idx = valid_idx[-1]
            trade_date = _extract_trade_date_from_row(out.loc[last_idx], date_col)
            return float(pct_values.loc[last_idx]), trade_date, "ak_stock_hk_hist_pct"
    ...
    trade_date = _extract_trade_date_from_row(out.iloc[-1], date_col)
    return (last_close / prev_close - 1.0) * 100.0, trade_date, "ak_stock_hk_hist_close_calc"


def fetch_hk_return_pct_akshare_daily(code, lookback_days=90):
    """使用 AKShare 港股历史日线获取最新交易日涨跌幅，兼容旧接口。"""
    r_pct, trade_date, source = fetch_hk_return_pct_akshare_daily_with_date(code=code, lookback_days=lookback_days)
    return r_pct, source
...
def fetch_hk_return_pct_last_close_with_fallback_with_date(code):
    """港股统一收盘口径的安全获取，并返回实际交易日。"""
    hk_code = normalize_hk_code(code)
    try:
        return fetch_hk_return_pct_akshare_daily_with_date(hk_code)
...
def fetch_hk_return_pct_last_close_with_fallback(code):
    """港股统一收盘口径的安全获取，兼容旧接口。"""
    r_pct, trade_date, source = fetch_hk_return_pct_last_close_with_fallback_with_date(code)
    return r_pct, source


def fetch_us_return_pct_akshare_daily_with_date(ticker):
    """
    使用 AKShare 美股历史日线获取最新交易日涨跌幅，并返回实际交易日。
...
def get_stock_return_pct(
    ...
    last_close 口径下 US/CN/HK 都缓存 trade_date。
    """
    market = str(market).strip().upper()
    ...
    cache = None
    item = None
    needs_trade_date_cache = (valuation_mode == "last_close")
        ...
        if item:
            try:
                if needs_trade_date_cache:
                    ...
                    if not item.get("trade_date") and market in {"CN", "HK"}:
                        pass
                        ...
                        _cache_log(f"使用文件行情缓存: {cache_key} -> {result[0]:+.4f}% trade_date={item.get('trade_date', '')}")
                        if return_trade_date:
    ...
    try:
        if valuation_mode == "last_close":
            if market == "US":
                ...
                try:
                    r_pct, fetched_trade_date, source = fetch_cn_security_return_pct_daily_with_date(str(ticker_norm).zfill(6))
                    result = (r_pct, source)
                except Exception:
                ...
                r_pct, fetched_trade_date, source = fetch_hk_return_pct_last_close_with_fallback_with_date(ticker_norm)
                result = (r_pct, source)
            else:
    ...
    if return_trade_date:
        return float(result[0]), result[1], _normalize_trade_date_key(fetched_trade_date)
    return result
...
def get_proxy_return_pct(
    ...
    if ctype in {"cn_etf", "cn_stock", "cn_security", "cn_fund"}:
        return get_stock_return_pct("CN", code, manual_returns_pct, prefer_intraday, us_realtime, hk_realtime, security_return_cache_enabled, cn_hk_hourly_cache, valuation_mode, return_trade_date=return_trade_date)
    if ctype in {"us_ticker", "us_stock", "us_etf"}:
        return get_stock_return_pct("US", code, manual_returns_pct, prefer_intraday, us_realtime, hk_realtime, security_return_cache_enabled, cn_hk_hourly_cache, valuation_mode, return_trade_date=return_trade_date)
    if ctype in {"hk_stock", "hk_etf", "hk_security"}:
        return get_stock_return_pct("HK", code, manual_returns_pct, prefer_intraday, us_realtime, hk_realtime, security_return_cache_enabled, cn_hk_hourly_cache, valuation_mode, return_trade_date=return_trade_date)
...
def estimate_stock_holdings_return(
    ...
    for _, row in df.iterrows():
        ...
        market_effective_flags.append(bool(r_pct is not None and not stale_zeroed))
...
def estimate_funds_and_save_table(
    ...
    auto_domestic_cache_enabled = _is_domestic_fund_table_context(
        title=title,
        output_file=output_file,
    )
    ...
    # 3. 作为 QDII / 全球基金本次估值交易日，用来判断 A股/港股是否应闭市置零。
    if auto_overseas_residual_enabled and benchmark_footer_items is None:
        benchmark_footer_items = get_us_index_benchmark_items(cache_enabled=cache_enabled)

    overseas_valuation_date = ""
    if auto_overseas_residual_enabled:
        overseas_valuation_date = _resolve_overseas_estimate_valuation_date(
    ...
    if stale_market_estimate_date is None:
        ...
        # 则 A股/港股只有在没有 4月1日行情时才置零；不能因为运行日是 4月2日而误置零。
    ...
    elif auto_domestic_cache_enabled:
        domestic_valuation_date = datetime.now().strftime("%Y-%m-%d")
        _write_domestic_fund_estimate_history_cache(
            result_df=result_df,
            detail_map=detail_map,
            valuation_date=domestic_valuation_date,
            title=title,
            output_file=output_file,
            cache_enabled=cache_enabled,
        )
...
if __name__ == "__main__":
    ...
    estimate_funds_and_save_table(
        ...
        holding_mode="auto",
        proxy_normalize_weights=False,
        us_realtime=False,  # 开启实时美股会拉全市场数据，耗时较长。
        hk_realtime=True,
        renormalize_available_holdings=True,
        include_purchase_limit=True,
        include_method_col=False,
