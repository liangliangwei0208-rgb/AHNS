...
def fetch_kr_return_pct_pykrx_daily_with_date(code, lookback_days=30):
    """
    使用 pykrx 获取韩国股票最新完整交易日日线涨跌幅，并返回实际交易日。
    ...
    说明
    ----
    pykrx 面向韩国市场，通常比 Yahoo Finance 更适合国内网络环境。
    如未安装 pykrx，请运行：pip install pykrx
    """
    ...
    df = krx_stock.get_market_ohlcv_by_date(start_date, end_date, kr_code)
    if df is None or df.empty:
        raise RuntimeError(f"pykrx 返回空数据: {kr_code}")

    out = df.copy().sort_index()

    # pykrx 常见列：开盘价, 最高价, 最低价, 收盘价, 成交量, 涨跌率
...
def fetch_kr_return_pct_yahoo_daily_with_date(code, lookback_days=15):
    """
    使用 Yahoo Finance 作为韩国股票日线兜底接口。

    注意：部分本地网络环境可能需要代理/VPN，因此该函数只作为 pykrx 后备方案。
    """
    kr_code = normalize_kr_code(code)
    symbol = f"{kr_code}.KS"
    ...
    if len(out) < 2:
        raise RuntimeError(f"Yahoo 日线有效数据不足: {symbol}")
...
def fetch_kr_return_pct_daily_with_date(code):
    """
    获取韩国股票最新完整交易日日线涨跌幅。
    ...
    额外规则：
        若本地运行日前一自然日是韩国 5月5日儿童节等已知休市日，
        且行情源最新真实交易日早于该休市日，则返回该休市日 0%。
        例如 2026-05-06 早间运行时，若最新真实交易日为 2026-05-04，
        则韩国股票 2026-05-05 当日涨跌幅返回 0%。
    """
    kr_code = normalize_kr_code(code)
    errors = []

    try:
        r_pct, trade_date, source = fetch_kr_return_pct_pykrx_daily_with_date(kr_code)
        return _apply_kr_holiday_zero_policy(r_pct, trade_date, source)
    except Exception as e:
        errors.append(f"pykrx: {repr(e)}")

    try:
        r_pct, trade_date, source = fetch_kr_return_pct_yahoo_daily_with_date(kr_code)
        return _apply_kr_holiday_zero_policy(r_pct, trade_date, source)
    except Exception as e:
...
def fetch_kr_return_pct_daily(code):
    """获取韩国股票最新完整交易日日线涨跌幅，兼容旧二元返回接口。"""
    r_pct, trade_date, source = fetch_kr_return_pct_daily_with_date(code)
    return r_pct, source
    ...
    try:
        if valuation_mode == "last_close":
            if market == "US":
                ...
                r_pct, fetched_trade_date, source = fetch_kr_return_pct_daily_with_date(ticker_norm)
                result = (r_pct, source)
            else:
