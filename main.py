...
def main() -> None:
    ...
    haiwai_valuation_date = resolve_haiwai_valuation_date(haiwai_benchmark_footer_items)
    ...
    estimate_funds_and_save_table(
        ...
        top_n=10,
        output_file="output/guonei_fund.png",
        title="国内市场收益率预估 " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        holding_mode="auto",
        proxy_normalize_weights=False,
        us_realtime=False,
        # 国内 / 港股通类基金使用盘中口径，适配白天手动运行。
        hk_realtime=True,
    ...
    log("国内基金持仓估算表格生成完成")
    image_paths.append("output/guonei_fund.png")
