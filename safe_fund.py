...
from tools.fund_universe import HAIWAI_FUND_CODES, GUONEI_FUND_CODES
...
def build_safe_display_df(records: list[dict[str, Any]]) -> pd.DataFrame:
    """
    将缓存记录转成公开展示表。
...
def save_guonei_safe_table(safe_df: pd.DataFrame, *, valuation_date: str) -> None:
    output_file = "output/safe_guonei_fund.png"
    # 复用原绘图函数的内部收益列名，同时在图片表头中展示为更克制的合规文案。
    image_df = safe_df.rename(columns={DISPLAY_OBSERVATION_COLUMN: ESTIMATE_RETURN_COLUMN})
    save_fund_estimate_table_image(
        result_df=image_df,
        output_file=output_file,
        title="国内基金模型估算观察 " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        display_column_names={ESTIMATE_RETURN_COLUMN: DISPLAY_OBSERVATION_COLUMN},
        footnote_text=(
            "依据基金季度报告前十大持仓股及指数估算，仅供学习记录，"
            "不构成投资建议；最终以基金公司更新为准。"
    ...
    add_risk_watermark(output_file)
    log(f"国内基金安全版预估表生成完成: {output_file}，缓存日期: {valuation_date}")
...
def build_and_save_guonei(cache: dict[str, Any]) -> bool:
    guonei_date, guonei_records = latest_market_records(
        cache,
        market_group="domestic",
        market_label="国内",
        fund_codes=GUONEI_FUND_CODES,
    )
    guonei_df = build_safe_display_df(guonei_records)

    save_guonei_safe_table(guonei_df, valuation_date=guonei_date)
    print("\n国内基金安全版预估表：")
    print(guonei_df.to_string(index=False))
    return True
...
def main() -> None:
    ...
    try:
        generated_any = build_and_save_guonei(cache) or generated_any
    except RuntimeError as exc:
        log(f"[WARN] 国内安全图未生成: {exc}")
