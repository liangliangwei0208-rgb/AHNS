...
- `tools/configs/workflow_configs.py`：维护 GitHub 流程和 Service 流程。无时间窗步骤属于完整日流程，始终运行；带 `run_window_bj` 的步骤只在命中北京时间窗口时运行。收盘观察图 `safe_fund.py` 仅在 06:00-13:40 运行；GitHub 流程不含富途夜盘，Service 流程额外包含富途夜盘。
...
`safe_fund.py` 是收盘观察图入口，只读正式缓存生成 `output/safe_haiwai_fund.png`；自动总入口只在 06:00-13:40 运行它，晚上盘前窗口不会再输出收盘观察图。
