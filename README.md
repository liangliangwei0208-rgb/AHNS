...
```text
.
├── git_main.py                  # GitHub/主机总控入口：实时窗口优先；非实时窗口运行完整日流程
...
- `tools/configs/workflow_configs.py`：维护 GitHub 流程和 Service 流程。命中盘前、盘中、盘后或富途夜盘实时窗口时，总入口只运行对应实时观察步骤；未命中实时窗口时，才运行完整日流程中符合 `run_window_bj` 的步骤。收盘观察图 `safe_fund.py` 仅在 06:00-13:40 运行；GitHub 流程不含富途夜盘，Service 流程额外包含富途夜盘。
...
Actions 运行 `python git_main.py`，使用 GitHub 流程：命中盘前/盘中/盘后窗口时只运行对应实时观察；未命中实时窗口时才运行完整日流程，其中收盘观察图只在 06:00-13:40 生成；不运行富途夜盘，也不自动生成 `first_pic.py`。
